...
import { t as require_react_dom } from "./react-dom-DWxzl2PR.js";
import { _ as axios, f as isAxiosError, m as mergeConfig$1, p as isCancel } from "./axios-ki550MeL.js";
...
function useIsomorphicLayoutEffect(effect, deps) {
	typeof window === "undefined" ? (0, import_react.useEffect)(effect, deps) : (0, import_react.useLayoutEffect)(effect, deps);
}
var isReact19 = typeof import_react.default.use === "function";
function usePage() {
	const page = isReact19 ? import_react.default.use(PageContext_default) : import_react.useContext(PageContext_default);
...
function useForm(...args) {
	...
	const setDataFunction = (0, import_react.useCallback)((keyOrData, maybeValue) => {
		if (typeof keyOrData === "string") setData((data2) => set(cloneDeep(data2), keyOrData, maybeValue));
		else if (typeof keyOrData === "function") setData((data2) => keyOrData(data2));
		else setData(keyOrData);
	}, [setData]);
	const [dataAsDefaults, setDataAsDefaults] = (0, import_react.useState)(false);
	...
	useIsomorphicLayoutEffect(() => {
		if (!dataAsDefaults) return;
		if (isDirty) setDefaults(data);
		setDataAsDefaults(false);
	}, [dataAsDefaults]);
	const reset = (0, import_react.useCallback)((...fields) => {
		if (fields.length === 0) setData(defaults);
		else setData((data2) => fields.filter((key) => has(defaults, key)).reduce((carry, key) => {
...
var Head = function({ children, title }) {
	...
	function renderNodes(nodes) {
		...
		if (title && !elements.find((tag) => tag.startsWith("<title"))) elements.push(`<title ${provider.preferredAttribute()}>${title}</title>`);
		return elements;
	}
...
function usePoll(interval, requestOptions = {}, options = {
	...
	const pollRef = (0, import_react.useRef)(router.poll(interval, requestOptions, {
		...options,
		autoStart: false
	}));
	(0, import_react.useEffect)(() => {
		if (options.autoStart ?? true) pollRef.current.start();
...
//#endregion
export { App, Deferred_default as Deferred, Form_default as Form, Head_default as Head, InfiniteScroll_default as InfiniteScroll, Link_default as Link, WhenVisible_default as WhenVisible, config, createInertiaApp, progress, router3 as router, useForm, useFormContext, usePage, usePoll, usePrefetch, useRemember };
